...
public class AssetModule extends NymModule {
	...
	public static String createAsset(String nymId, String definition) throws OTException {
		...
		String assetName = extractFromContract(definition, "currency name");
		List<String> assetIds = getAssetIds();
		for (String assetId : assetIds) {
			if (assetName.equalsIgnoreCase(getAssetName(assetId)))
				error("This asset is already in the wallet");
		}
	...
	public static String addAsset(String contract) throws OTException {
		...
		// /how to check if asset is already in the wallet??
		String assetName = extractFromContract(contract, "currency name");
		List<String> assetIds = getAssetIds();
		for (String assetId : assetIds) {
			if (assetName.equalsIgnoreCase(getAssetName(assetId)))
				error("This asset is already in the wallet");
		...
		if (!Util.isValidString(assetId))
			error("The new asset not found in the wallet");
		showAsset(assetId);
	...
	public void queryAsset() throws OTException {
		...
		Storable storable1 = OTAPI.SwigStorage.createObject(StoredObjectType.STORED_OBJ_STRING_MAP);
		if (storable1 == null)
		...
		final String encodedMap = OTAPI.SwigStorage.encodeObject(map1);
		...
		Storable storable2 = OTAPI.SwigStorage.decodeObject(StoredObjectType.STORED_OBJ_STRING_MAP, replyMap);
		if (storable2 == null)
	...
	public void showPurse(String purse) throws OTException {
		...
		for (int index = 0; index < size; index++) {
			...
			print(String.format("%5d: %15s | %2d | %s | %s | %s",
					++i, convertAmountToFormat(assetId, denomination), series,
					UTC.dateToString(validFrom), UTC.dateToString(validTo), status));
		}
		print(String.format("%s: %15s", "Total", convertAmountToFormat(assetId, balance)));
		print(Util.repeat("-", 13));
	}
