...
public class NymModule extends ServerModule {
	...
	public String issueAsset(final String contract, String accountName) throws OTException {
		...
		String signerId = extractFromContract(contract, "nymID");
		if (!nymId.equals(signerId))
			error("Asset contract signer and issuer must be the same");
		
		if (!isNymRegisteredAtServer(serverId, nymId))
			error("Nym needs to be registered at the server");
