package net.otxs.bridge.core.modules;
...
import net.otxs.bridge.UTC;
import net.otxs.bridge.Util;
import net.otxs.bridge.core.DataModel;
import net.otxs.bridge.core.Settings;
import net.otxs.bridge.core.commands.Commands.Extension;
import net.otxs.bridge.core.commands.Commands.Sophistication;
import net.otxs.bridge.core.dto.Account;
import net.otxs.bridge.core.dto.Transaction.InstrumentType;
import net.otxs.bridge.core.exceptions.OTException;
import net.otxs.bridge.core.exceptions.OTSystemException;
import net.otxs.bridge.core.exceptions.OTSystemException.Event;
import net.otxs.bridge.core.exceptions.OTUserException;
...
public abstract class Module {
	...
	public static String getWalletFileName(String walletId) {
		return String.format("%s/%s.%s", ApplProperties.getUserDataPath(),
				walletId, Extension.DEFINITION.getValue());
	}

	public static void createWallet(String walletId) {
		try {
			InputStream is = ClassLoader.getSystemResource(ApplProperties.get().getString("template.wallet")).openStream();
	...
	public static void loadWallet(String walletId) throws OTException {
	...
	public static AccountType getAccountType(String accountId) {
		if (!Util.isValidString(accountId))
			return null;
		return AccountType.parse(OTAPI.GetAccount.type(accountId));
	}

	public static String getAccountStandardName(String accountId) {
		if (getAccountType(accountId).equals(AccountType.ISSUER)) {
	...
	protected static String extractFromContract(String contract, String tag) throws OTException {
		int index = contract.indexOf(String.format("%s=", tag));
		if (index == 0)
			error(String.format("%s: %s", "Unable to find tag inside contract", tag));
		int beg = contract.indexOf("\"", index) + 1;
		int end = contract.indexOf("\"", beg);
		return contract.substring(beg, end);
	}
