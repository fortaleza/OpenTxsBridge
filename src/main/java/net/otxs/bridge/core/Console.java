...
public class Console extends JTextPane {
	...
	private class ConsoleDocument extends DefaultStyledDocument {
		...
		public void insertString(int offset, String s, AttributeSet a) throws BadLocationException {
			...
			if (isPasteOn) {
				s = paste = Interpreter.escapeNewLines(getClipboardContents());
				isPasteOn = false;
			}
			if (s.equals("\n"))// lg6
				offset = getLength();
			if (offset >= changeLimit && getLength() >= changeLimit) {
				if (name.equals("out") && s.equals("{")) {
					StyleConstants.setForeground(styleOut, 
							new Color(ApplProperties.get().getInteger("console.color.alt")));
				} else if (name.equals("out") && s.equals("}")) {
					StyleConstants.setForeground(styleOut, 
							new Color(ApplProperties.get().getInteger("console.color.out")));
				} else
	...
	public void setInColor(Color color) {
		StyleConstants.setForeground(styleIn, color);
	}
