...
public class NativeLoader {
	...
	protected static Logger logger =
			Logger.getLogger(NativeLoader.class.getName());
	...
	static {
		...
		for (int i = 0; i < entries.length; i++) {
			...
			nativeLibMap.put(key[0].trim() + ApplProperties.get().getString(
					"nativeLib.keySeparator") + key[1].trim(),
					keyValue[1].trim());
		}
	}
	...
	private static void loadDll(String libName) {
		...
		URL url = null;
		try {
			for (urls = ClassLoader.getSystemResources(libName + ext); urls
					.hasMoreElements();) {
				url = urls.nextElement();
				if (url.toString().indexOf(folder) != -1)
					break;
			}
			if (url != null) {/** library is jarred */
				String packagen = NativeLoader.class.getPackage().getName().
						replaceAll("\\.", "/");
				InputStream in = url.openStream();
				/**
				 Attempted to store files in the 'temporary' folder, however,
				 it does not work as the temporary file names are decorated
				 with a code.
			...
			} else {/** library is not jarred, perhaps, it is there */
				System.load(NativeLoader.class.getResource(
						folder + "/" + libName + ext).getPath());
			}
		} catch (Exception e) {
