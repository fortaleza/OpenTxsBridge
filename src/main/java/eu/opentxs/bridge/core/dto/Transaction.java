...
public class Transaction implements Comparable<Transaction> {
	...
	public enum TransactionType {
    	...
    	INSTRUMENT_NOTICE("instrumentNotice"), 
    	TRANSFER_RECEIPT("transferReceipt"),
    	VOUCHER_RECEIPT("voucherReceipt"),
    	CHEQUE_RECEIPT("chequeReceipt"),
	...
	private Transaction(String refNum, String dateSigned, 
			String senderNymId, String senderAccountId,
			String recipientNymId, String recipientAccountId, String hisNymId,
			String trxnNum, String instrument, InstrumentType instrumentType, TransactionType trxnType, 
			Side side, Status status, Double volume, String note,
	...
	public static List<Transaction> getListOfOutpayments(
			String serverId, String nymId, String assetId, int size, InstrumentType[] instrumentTypes) throws Exception {
		...
		for (int index = 0; index < size; index++) {
			...
			list.add(new Transaction(refNum, dateSigned, 
					senderNymId, senderAccountId, recipientNymId, recipientAccountId, hisNymId,
					trxnNum, instrument, instrumentType, trxnType, side, status, volume, note, validFrom, validTo));
		}
		return list;
	}
	
	public static List<Transaction> getListForNym(
			String serverId, String nymId, String assetId,
			String ledger, int size) throws Exception {
		List<Transaction> list = new ArrayList<Transaction>();
		for (int index = 0; index < size; index++) {
			...
			String transaction = OTAPI.Ledger.getTransactionByIndex(
					serverId, nymId, nymId, ledger, index);
			if (!Util.isValidString(transaction))
				Module.error("transaction is empty");
			
			String refNum = OTAPI.Transaction.getDisplayReferenceToNum(
					serverId, nymId, nymId, transaction);
			String dateSigned = OTAPI.Transaction.getDateSigned(
					serverId, nymId, nymId, transaction);
			
			String senderNymId = OTAPI.Transaction.getSenderNymId(
					serverId, nymId, nymId, transaction);
			String senderAccountId = null;
			String recipientNymId = OTAPI.Transaction.getRecipientNymId(
					serverId, nymId, nymId, transaction);
			String recipientAccountId = null;
			String hisNymId = getHisNymId(nymId, senderNymId, recipientNymId);
			...
			String trxnNum = OTAPI.Instrument.getTransactionNum(instrument);
			InstrumentType instrumentType = Module.getInstrumentType(instrument);
			TransactionType trxnType = TransactionType.parse(
					OTAPI.Transaction.getType(serverId, nymId, nymId, transaction));
			Side side;
	...
	public static List<Transaction> getListForAccount(
			String serverId, String nymId, String accountId,
			String ledger, int size) throws Exception {
		List<Transaction> list = new ArrayList<Transaction>();
		for (int index = 0; index < size; index++) {
			String transaction = OTAPI.Ledger.getTransactionByIndex(
			...
			String refNum = OTAPI.Transaction.getDisplayReferenceToNum(
					serverId, nymId, accountId, transaction);
			String dateSigned = OTAPI.Transaction.getDateSigned(
					serverId, nymId, accountId, transaction);
			String senderNymId = OTAPI.Transaction.getSenderNymId(
					serverId, nymId, accountId, transaction);
			String senderAccountId = OTAPI.Transaction.getSenderAccountId(
					serverId, nymId, accountId, transaction);
			String recipientNymId = OTAPI.Transaction.getRecipientNymId(
					serverId, nymId, accountId, transaction);
			String recipientAccountId = OTAPI.Transaction.getRecipientAccountId(
					serverId, nymId, accountId, transaction);
			...
			String trxnNum = null;
			TransactionType trxnType = TransactionType.parse(OTAPI.Transaction.getType(
					serverId, nymId, accountId, transaction));
			...
			Double volume = Module.getDouble(OTAPI.Transaction.getAmount(
					serverId, nymId, accountId, transaction));
	...
	public static List<Transaction> getListForOutbox(
			String serverId, String nymId, String accountId,
			String ledger, int size) throws Exception {
		List<Transaction> list = new ArrayList<Transaction>();
		for (int index = 0; index < size; index++) {
			...
			Double volume = 0 - Module.getDouble(OTAPI.Transaction.getAmount(
					serverId, nymId, accountId, transaction));
	...
	public static void showH(List<Transaction> list) {
		...
		for (Transaction transaction : list) {
			Module.print(String.format("%18s : %-8s : %-8s : %9.2f : %2s : %s %s",
					//%5s | %5s |
					//transaction.refNum, transaction.trxnNum,
					UTC.timeToString(transaction.dateSigned),
					transaction.instrumentType, transaction.side,
					transaction.volume, transaction.status,
	...
	public static void present(List<Transaction> list) {
		...
		int i = 0;
		for (Transaction transaction : list) {
			Module.print(String.format("%3d: %18s | %-8s | %-8s | %9.2f | %2s | %s", ++i,
					UTC.timeToString(transaction.dateSigned),
					transaction.instrumentType, transaction.side,
					transaction.volume, transaction.status,
					Util.crop(ContactModule.getContactName(transaction.hisNymId), 15)));
	...
	public void print() {
		...
		if (Util.isValidString(validFrom))
			Module.print(String.format("%12s: %s", "ValidFrom", UTC.timeToString(validFrom)));
		else
			Module.print(String.format("%12s:", "ValidFrom"));
		
		if (Util.isValidString(validTo))
			Module.print(String.format("%12s: %s", "ValidTo", UTC.timeToString(validTo)));
		else
			Module.print(String.format("%12s:", "ValidTo"));
			
		if (Util.isValidString(senderNymId))
			Module.print(String.format("%12s: %s (%s)", "NymFrom", senderNymId,
					ContactModule.getContactName(senderNymId)));
		else
			Module.print(String.format("%12s:", "NymFrom"));
		
		if (Util.isValidString(senderAccountId))
			Module.print(String.format("%12s: %s (%s)", "AccountFrom", senderAccountId, 
					ContactModule.getContactAccountName(senderAccountId)));
		else
			Module.print(String.format("%12s:", "AccountFrom"));
		
		if (Util.isValidString(recipientNymId))
			Module.print(String.format("%12s: %s (%s)", "NymTo", recipientNymId,
					ContactModule.getContactName(recipientNymId)));
		else
			Module.print(String.format("%12s:", "NymTo"));
		
		if (Util.isValidString(recipientAccountId))
			Module.print(String.format("%12s: %s (%s)", "AccountTo", recipientAccountId, 
					ContactModule.getContactAccountName(recipientAccountId)));
		else
	...
	private static String getHisNymId(String nymId, String senderNymId, String recipientNymId) {
		...
		if ((nymId.equals(senderNymId) || !Util.isValidString(senderNymId)) 
				&& !nymId.equals(recipientNymId))
			hisNymId = recipientNymId;
		else if ((nymId.equals(recipientNymId) || !Util.isValidString(recipientNymId)) 
				&& !nymId.equals(senderNymId))
	...
	private static String getHisAccountId(String accountId, String senderAccountId, String recipientAccountId) {
		...
		if ((accountId.equals(senderAccountId) || !Util.isValidString(senderAccountId)) 
				&& !accountId.equals(recipientAccountId))
			hisAccountId = recipientAccountId;
		else if ((accountId.equals(recipientAccountId) || !Util.isValidString(recipientAccountId)) 
				&& !accountId.equals(senderAccountId))
